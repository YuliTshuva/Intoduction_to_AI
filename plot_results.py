...
ax[0].bar(list(accuracies.keys()), list(accuracies.values()),
          color=colors,
          edgecolor="black")
ax[0].set_title("Accuracy of the models", fontsize=18)
...
for model_name, model in models.items():
    ...
    i += 1
ax[1].set_title("ROC curve of the models", fontsize=18)
...
# Final adjustments
plt.tight_layout()
plt.savefig(join(PLOTS_DIR, "results.png"), dpi=1000)
plt.show()

# Plot the feature importance of the models
fig, ax = plt.subplots(1, 3, figsize=(16, 10))
fig.suptitle("Feature Importance of the models", fontsize=25, y=0.96)
i = 0
for model_name, model in models.items():
    ...
    ax[i].barh(pos, feature_importances[sorted_idx], align='center', color=colors[i])
    ax[i].set_yticks(pos)
    ax[i].set_yticklabels(np.array(X_test.columns)[sorted_idx])
    ax[i].set_xlabel('Relative Importance')
    ax[i].set_title(f'{model_name}', fontsize=19)
    i += 1

# Final adjustments
plt.tight_layout(rect=[0.03, 0.03, 0.95, 0.95])
plt.savefig(join(PLOTS_DIR, "feature_importance.png"), dpi=1000)
plt.show()
